...
USING (
  SELECT * FROM UNNEST([
    ...
    STRUCT ('Nymble', 'Lokix'),
    STRUCT ('Pawmi', 'Pawmo'),STRUCT ('Pawmo', 'Pawmot'), --Same evolution included in different syntax earlier to test redundancy paramaters
...
USING (
  SELECT * FROM UNNEST([
    ...
    STRUCT('Nidoran' AS from_species, 'Nidorina' AS to_species),
    STRUCT('Nidorina' AS from_species, 'Nidoqueen' AS to_species),

    STRUCT('Nidoran' AS from_species, 'Nidorino' AS to_species),
